...
#define OUTPUT_DATA_RATE 10

const uint32_t RATE_B_PER_SEC = 800*3*2; // ODR is 800 Hz, one data output is 6*2 bytes

const uint32_t TEST_TIME_SEC = 60; // Max time 

const uint32_t BLOCK_COUNT = (RATE_B_PER_SEC*TEST_TIME_SEC + 511)/512; // Number of blocks
...
void initializeAccel(SparkFun_KX132_SPI &accel, byte csPin) {
    ...
    accel.setOutputDataRate(OUTPUT_DATA_RATE); 
    accel.enableAccel();
    delay(5);
...
void createDataFile(char *filename) {
    sd.remove(filename);
    if (!file.createContiguous(filename, 512UL*BLOCK_COUNT)) {
